...
export default function Searchbar() {
        ...
        products.forEach((product) => {
            if (
                !categoryStringtArr.find(
                    (categoryStringtArr) =>
                        categoryStringtArr === product.category_type
                )
            ) {
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        if (categories.length !== 0) {
            ...
            searchParams.set(
                'categories',
                makeCategoryStringArr(
                    makeUniqueCategoryArr(products, store_type)
                ).join(',')
            );
        }
                ...
                <form onSubmit={handleSubmit}>
                    <fieldset className="flex flex-col items-start">
                        ...
                        >
                            ...
                            {makeUniqueCategoryArr(products, store_type).map(
                                (product) => {
                                    return (
                                        <label
                                            htmlFor="category"
