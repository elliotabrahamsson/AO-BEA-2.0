        ...
        fetch("http://localhost:3000/products")
            ...
            .then((data: Product[])=> {
                ...
                const filteredProducts = data.filter((product) => {
                    if(store_type === "Man") {
                        if (product.gender === "Man" && product.category_type === selected_category){
                            return true;
                        }

                    }else if (store_type === "Woman"){
                        if (product.gender === "Woman" && product.category_type === selected_category){
                            return true;
                        }
                    }
                    console.log("store_type", store_type, "selected_category", selected_category);
                    return false;
                });
